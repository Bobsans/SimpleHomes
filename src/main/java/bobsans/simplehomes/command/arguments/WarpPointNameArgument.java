...
public class WarpPointNameArgument implements ArgumentType<String> {
    ...
    private Collection<String> userWarpPoints(PlayerEntity player) {
        return PlayerDataManager.load().getOrCreate(player).warps.values().stream().map((warp) -> warp.name).collect(Collectors.toList());
    }
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        if (context.getSource() instanceof CommandSource) {
            try {
                return ISuggestionProvider.suggest(userWarpPoints(((CommandSource) context.getSource()).asPlayer()), builder);
